...
import VirtualProfessor from "@/components/learning/VirtualProfessor";
import ScanAndSolve from "@/components/learning/ScanAndSolve";
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8 pb-32">
      ...
      <div className="animate-fade-in transition-all duration-500" key={activeTab}>
        ...
        {activeTab === "solve" && <ScanAndSolve />}
        {activeTab === "chat" && <ChatTab />}
      </div>

      <VirtualProfessor summary={currentSession.data.research?.wiki || "I've analyzed your session on " + currentSession.title + ". We'll be covering key concepts, interactive quizzes, and a visual knowledge map to ensure mastery. Let's start with the core notes."} />
