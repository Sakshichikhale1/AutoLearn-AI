...
function MindMapFlow({ content, onImageCapture }: MindMapTabProps) {
  ...
  const handleExpand = async () => {
    ...
    try {
      ...
      data.sub_nodes.forEach((label: string, i: number) => {
        const id = `exp-${selectedNode.id}-${i}-${Date.now()}`;
        newNodes.push({
          id,
          data: { label },
          position: { x: selectedNode.position.x + (i - 1) * 200, y: selectedNode.position.y + 100 },
          style: { backgroundColor: '#f0f9ff', color: '#0369a1', fontWeight: 'bold', fontSize: '11px', padding: '8px', borderRadius: '8px', border: '1px solid #bae6fd', width: nodeWidth - 40, textAlign: 'center' }
        });
        newEdges.push({
          id: `e-${selectedNode.id}-${id}`,
          source: selectedNode.id,
          target: id,
          style: { stroke: "#bae6fd", strokeWidth: 1.5 },
          animated: true
        });
      });
      ...
      toast.success("AI expanded this concept!");
    } catch (e) {
      toast.error("Could not expand concept");
    } finally {
      setExpanding(false);
    }
  };
      ...
      >
        ...
        {selectedNode && (
          <div className="absolute bottom-4 right-4 z-20 w-80 bg-slate-900 text-white p-5 rounded-3xl shadow-2xl border border-white/10 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {explanation ? (
              <div className="max-h-64 overflow-y-auto mb-4 pr-2 custom-scrollbar text-xs text-slate-300 leading-relaxed prose prose-invert prose-p:leading-relaxed prose-headings:text-white prose-headings:font-black prose-headings:text-sm">
                <ReactMarkdown>
                  {explanation}
                </ReactMarkdown>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Button 
                  ...
                  disabled={asking || expanding}
                  variant="outline"
                  className="w-full h-10 rounded-xl gap-2 font-bold bg-white/5 border-white/10 text-white hover:bg-white/10 transition-all active:scale-95"
                >
                  {expanding ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4 text-yellow-400" />}
                  {expanding ? "Expanding..." : "AI Sub-Node Expand"}
                </Button>
              </div>
            )}
