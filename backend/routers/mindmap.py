...
async def expand_node(req: ExpandRequest):
    try:
        prompt = f"""Given the concept '{req.node_label}' in the context of:
        {req.context[:4000]}
        
        Provide exactly 3 more detailed sub-concepts that branch off from '{req.node_label}' for a mind map.
        Labels must be 2-4 words maximum.
        
        Return STRICT JSON:
        {{
          "sub_nodes": ["Concept A", "Concept B", "Concept C"]
        }}"""
        ...
        raise HTTPException(500, f"Node expansion failed: {str(e)}")
