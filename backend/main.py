...
# --- Functions (unchanged) ---
# get_academic_papers is now in routers/research.py
...
async def generate_learning_content(
    ...
    try:
        print(f"DEBUG: Learning endpoint hit with text={bool(text)}, files_count={len(files)}")
        content = ""
        if text:
            content += text + "\n"
        ...
        for file in files:
            ...
            if filename.endswith(".pdf"):
                        ...
                        messages=[
                            {
                                "role": "user",
                                "content": [
                                    {"type": "text", "text": "Transcribe all text from this image and describe any diagrams or important visual information in detail for educational purposes."},
                                    {
                                        "type": "image_url",
                                        "image_url": {
                ...
                vision_resp = await asyncio.to_thread(call_groq_vision)
                content += f"\n[Image Content from {file.filename}]:\n" + vision_resp.choices[0].message.content + "\n"
                ...
                whisper_resp = await asyncio.to_thread(call_groq_whisper)
        ...
        prompt = f"""You are an elite academic tutor. Analyze the provided multimodal content (text, transcripts, and image descriptions) to create a comprehensive, premium study suite.
        
        The content comes from multiple sources. Synthesize everything into a cohesive learning experience.
        
        Return STRICT JSON:
        {{
          "notes": [
            {{"title": "Section Title", "content": "Detailed explanatory text with key concepts highlighted"}}
          ],
          "quiz": [
            {{"question": "...", "options": ["A", "B", "C", "D"], "correct": 0, "explanation": "Detailed explanation of why the answer is correct and why others are wrong."}}
          ],
          "flashcards": [
            {{"q": "Core Question/Concept", "a": "Detailed Answer/Explanation"}}
          ],
          "vocabulary": [
            {{"word": "Term", "definition": "Formal academic definition"}}
          ],
...
async def chat_tutor(
    ...
    try:
        ...
        system_prompt = f"""You are an advanced AI Learning Assistant. Use Markdown.
